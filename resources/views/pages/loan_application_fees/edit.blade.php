        ...
        <div class="row">
            ...
            <div class="col-4">
                <div class="card mb-4">
                    ...
                    <div class="card-body" style="height: 500px">
                        <p><b>Amount Needed:</b> {{$fees->payment->amount}}</p>
                        <p><b>Title:</b> {{$fees->payment->title}}</p>
                        <p><b>Amount:</b> {{$fees->payment->amount}}</p>
                        <p><b>Payment Reference:</b> {{$fees->payment->payment_ref}}</p>
                        <p><b>Payment Method:</b> {{$fees->payment->payment_method}}</p>
                        <p><b>Created At:</b> {{$fees->payment->created_at}}</p>

                        <h6 class="text-primary">ACTION FORM</h6>
                        <hr>

                        <form action="{{url("payments/application_fees/{$fees->id}/editSubmit")}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group mb-2">
                                <label for="exampleInputEmail1">Loan Status</label>
                                <select name="status" class="form-control" id="exampleInputEmail1" >
                                    <option
                                        @if($fees->payment->status=="pending") selected @endif
                                    value="pending">Pending</option>
                                    <option
                                            @if($fees->payment->status=="approved") selected @endif
                                    value="approved">Approved</option>
                                    <option
                                            @if($fees->payment->status=="declined") selected @endif
